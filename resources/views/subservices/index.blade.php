              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td><img class="center-block" src="{{ asset($subservice->Logo) }}" height="100px" width="100px" /></td>
                      
                      <td>{{ $subservice->Enname }}</td>
                      <td>{{ $subservice->Price }}</td>
                      <td>{{ $subservice->Endescription }}</td>
                      <td>{{ $subservice->Eninstrucation }}</td>
                       <td>
                      ...
                      <td>
                        <form action="{{ route('subservices.destroy',$subservice->id) }}" method="POST">
                          <a class="btn btn-info" href="{{ route('subservices.show',$subservice->id) }}">Show</a>
                          @csrf
                          @method('DELETE')
